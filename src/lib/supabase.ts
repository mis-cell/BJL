...
if (supabase) {
  Promise.resolve().then(async () => {
      ...
      await supabase.rpc('exec_sql', {
        query: `
          ...
          BEGIN
            ...
            IF EXISTS (SELECT FROM pg_tables WHERE schemaname = 'public' AND tablename = 'mill_inspection_master') THEN
              EXECUTE 'DROP VIEW IF EXISTS material_inspection CASCADE';
              EXECUTE 'CREATE OR REPLACE VIEW material_inspection AS SELECT * FROM mill_inspection_master';
            END IF;
    ...
    // Independent Satta table & column provisioning to bypass monolith transaction block errors and guarantee existence
    const schemaQueries = [
