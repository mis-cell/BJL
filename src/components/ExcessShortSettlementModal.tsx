...
export const ExcessShortSettlementModal: React.FC<ExcessShortSettlementModalProps> = ({
  ...
  // 4. Grade Consolidation Across All Arrivals
  const gradeBreakdown = useMemo(() => {
    ...
    // Get contract rates from Sauda details
    const clean = (s: any) => String(s || '').trim().toUpperCase();
    const poDetails = (allScpDetails || []).filter((d: any) => clean(d.po_no) === clean(poNo));
    ...
    linkedFinalArrivals.forEach((ar: any) => {
      ...
      if (Array.isArray(parsedGrid) && parsedGrid.length > 0) {
        parsedGrid.forEach((row: any) => {
          const gName = String(row.receipt_grade_name || row.challan_grade_name || row.item_name || row.grade || 'TD10').trim().toUpperCase();
          const marka = String(row.challan_marka_name || row.marka_name || row.marka || '--').trim().toUpperCase();
          const crop = String(row.crop_year || '--').trim();
          ...
          if (!map[gName]) {
            // Find rate in poDetails
            const matchDet = poDetails.find((d: any) => clean(d.grade_code || d.grade_name || d.quality) === clean(gName));
            const rQtl = matchDet ? Number(matchDet.rate_qntl || matchDet.rate || matchDet.rs || 0) : 0;
    ...
    const list = Object.values(map).sort((a, b) => b.totalWeightMt - a.totalWeightMt);
    return list;
  }, [linkedFinalArrivals, allScpDetails, poNo]);
  ...
  const lastArrivalDate = useMemo(() => {
    ...
    return last.voucher_date || last.arrival_date || last.date || new Date().toISOString().split('T')[0];
  }, [linkedFinalArrivals]);
  ...
  const [saudaRateQtl, setSaudaRateQtl] = useState<number>(0);
  const [sattaRateQtl, setSattaRateQtl] = useState<number>(0);
  const [rateBasis, setRateBasis] = useState<'min_sauda_satta' | 'sauda_rate' | 'satta_rate' | 'manual'>('min_sauda_satta');
  ...
  useEffect(() => {
    if (gradeBreakdown.length > 0 && !selectedGrade) {
      setSelectedGrade(gradeBreakdown[0].grade);
    }
  }, [gradeBreakdown]);

  // Sync Sauda & Satta rates when selected grade or date changes
  useEffect(() => {
    ...
    if (saudaRate === 0) {
      // Fallback search in allScpDetails or default market range
      const det = (allScpDetails || []).find((d: any) => 
        String(d.po_no).trim().toUpperCase() === String(poNo).trim().toUpperCase() &&
        (String(d.grade_code || d.quality).trim().toUpperCase() === curGrade)
      );
      if (det) {
    ...
    // Look up Satta rate for lastArrivalDate and curGrade
    let foundSattaRate = 0;
    if (sattaCalculatedRates && sattaCalculatedRates.length > 0) {
      const match = sattaCalculatedRates.find((s: any) => 
        String(s.grade || '').trim().toUpperCase() === curGrade &&
  ...
  }, [selectedGrade, gradeBreakdown, lastArrivalDate, allScpDetails, poNo, sattaCalculatedRates, sattaBaseRates]);
  ...
  // Determine Applicable Rate (per Quintal) based on Policy
  const effectiveRateQtl = useMemo(() => {
    ...
    // Policy rule: Use MIN(Sauda Rate, Last Arrival Satta Rate)
    return Math.min(saudaRateQtl, sattaRateQtl);
  }, [rateBasis, saudaRateQtl, sattaRateQtl, manualRateQtl]);

  const effectiveRateMt = effectiveRateQtl * 10;

  // Determine Deduction Quantity (in MT)
  ...
  const totalDeductionAmount = useMemo(() => {
    return Math.round(deductionQtyQtl * effectiveRateQtl * 100) / 100;
  }, [deductionQtyQtl, effectiveRateQtl]);
  ...
  const handleSaveDeduction = async () => {
    ...
    const payload = {
      ...
      applicable_rate: effectiveRateQtl,
    ...
    try {
      ...
      setSaveMessage("✓ Excess / Short Settlement calculation saved and linked successfully!");
    ...
    <div className="fixed inset-0 z-[1000] bg-slate-900/70 backdrop-blur-xs flex items-center justify-center p-3 sm:p-6 overflow-y-auto animate-in fade-in duration-200">
      <div className="bg-white w-full max-w-5xl rounded-2xl shadow-2xl border border-slate-200 flex flex-col max-h-[92vh] overflow-hidden my-auto text-slate-900">
        ...
        <div className="flex-1 overflow-y-auto p-5 space-y-5 bg-slate-50/50">
          ...
          {/* Section 3: Grade / Quality Consolidation & Deduction Grade Selection */}
          <div className="bg-white rounded-xl p-4 border border-slate-200 shadow-xs space-y-3">
            <div className="flex items-center justify-between border-b border-slate-100 pb-2">
              <div className="flex items-center gap-2">
              ...
              <div className="bg-amber-100/60 p-3 rounded-lg border border-amber-300 flex flex-col justify-between">
                <div className="flex items-center justify-between">
                  ...
                  <span className="text-[8.5px] font-bold px-1.5 py-0.2 rounded bg-amber-200 text-amber-900 uppercase">
                    {sattaRateQtl < saudaRateQtl ? 'Satta Lower ✓' : 'Sauda Lower ✓'}
                  </span>
                </div>
                <div className="my-1">
                  <span className="text-xl font-black text-amber-950 font-mono">
                    ₹{effectiveRateQtl.toLocaleString()}
                  </span>
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 gap-4 bg-white p-4 rounded-xl border border-amber-200 shadow-2xs">
              <div className="space-y-2">
                ...
                <div className="flex flex-wrap gap-2">
                  ...
                  >
                    <span>Excess Over Tolerance (+{tolerance.excessOverToleranceMt.toFixed(3)} MT)</span>
                  </button>
                  <button
                    type="button"
                  ...
                  >
                    <span>Excess Over Contract (+{tolerance.excessOverContractMt.toFixed(3)} MT)</span>
                  </button>
                  <button
                    type="button"
                    onClick={() => {
              ...
              <div className="bg-gradient-to-r from-slate-900 to-indigo-950 text-white p-4 rounded-xl flex flex-col justify-between shadow-xs">
                ...
                <div className="text-[9.5px] text-slate-300 font-mono">
                  Calculation: {deductionQtyQtl.toFixed(2)} Qtl × ₹{effectiveRateQtl.toLocaleString()}/Qtl
                </div>
              </div>
            </div>
            ...
            <div className="grid grid-cols-1 sm:grid-cols-3 gap-3">
              <div className="sm:col-span-2">
                ...
                <input 
                  ...
                  placeholder="e.g. Excess weight 5.609 MT deducted at spot Satta rate of last arrival dated 07/04/2026 for TD10."
        ...
        {/* Modal Footer */}
        <div className="px-6 py-3.5 bg-slate-100 border-t border-slate-200 flex items-center justify-between select-none">
          <div className="text-[11px] text-slate-500 flex items-center gap-2">
            <Info className="w-4 h-4 text-slate-400" />
            <span>Updates database records for <strong>Sauda Check Point</strong> & <strong>Final P.O</strong> automatically.</span>
          </div>
          <div className="flex items-center gap-2.5">
            ...
            <button
              ...
              disabled={isSaving}
              className="px-5 py-2 rounded-xl bg-amber-600 hover:bg-amber-700 text-white text-xs font-black shadow-md transition flex items-center gap-2 cursor-pointer active:scale-95 disabled:opacity-50"
            >
