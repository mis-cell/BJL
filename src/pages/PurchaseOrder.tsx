  ...
  const handleCsvDownload = async () => {
    try {
      ...
      const dataToExport = listToExport.map((row: any) => {
        ...
        const pStatus = row.status ? String(row.status).toUpperCase() : (row.pending === false ? 'COMPLETED' : 'PENDING');
...
function isPoMismatchResolved(poNo: string): boolean {
  ...
  const fetchPosAndMasters = async () => {
    ...
    try {
      ...
      setPoList((pos || []).map((p: any) => {
        ...
        const isExplicitCompleted = p.status === 'completed';
        const isWeightCompleted = contractWeight > 0 && totalReceivedMt >= (contractWeight - 0.05);
  ...
  const filteredPos = poList.filter(p => {
    ...
    const isCompleted = p.pending === false || pendingStr === 'no' || pendingStr === 'false' || p.pending === 0 || statusStr === 'completed' || statusStr === 'settled' || (contractWt > 0 && receivedWt >= contractWt);
    const computedStatus = isCompleted ? 'completed' : (receivedWt > 0 ? 'partial' : 'pending');
  ...
  const totalCompletedPos = scopedPos.filter(p => {
    ...
    const receivedWt = parseFloat(p.received_weight_mt) || 0;
    const contractWt = parseFloat(p.total_contract_mt) || 0;
    return p.pending === false || pendingStr === 'no' || pendingStr === 'false' || p.pending === 0 || statusStr === 'completed' || statusStr === 'settled' || (contractWt > 0 && receivedWt >= contractWt);
  }).length;
        ...
        <LegacyLayout title={isTempPo ? "SAUDA CHECK POINT" : "FINAL P.O"} subtitle={isTempPo ? "Purchase Order Generation Mode" : "Purchase Order Generation Mode"} onBack={() => setViewMode('register')} onClose={() => setViewMode('register')}>
          <div ref={poFormRef} className="space-y-4 text-[11px] font-bold">
            ...
            <div className="bg-[#d4d0c8] p-1 border-t border-slate-400 flex flex-wrap justify-center items-center gap-1.5 shadow-[0_2px_4px_rgba(0,0,0,0.1)]">
               ...
               {/* Change Add Button to "View All P.O" Just Like Sauda View */}
               <LegacyButton 
                 icon={FileText} 
                 label="View All P.O" 
