...
async function verifyAndMigrateSupabaseSchema() {
  console.log("Checking and updating Supabase database table fields & schemas...");
  const migrationQuery = `
    -- Schema Migrations & Alterations for New Rate & Premium Fields
  ...
  try {
    await supabase.rpc('exec_sql', { query: migrationQuery });
    console.log("Supabase table fields & schema migrations applied successfully!");
  } catch (err: any) {
    console.warn("Notice during Supabase schema migration via RPC:", err?.message || err);
  }
}
...
async function syncEmailsBackground() {
  ...
  // Also run schema migrations
  await verifyAndMigrateSupabaseSchema();
